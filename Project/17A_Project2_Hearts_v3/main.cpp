...
void trick(Player &p, Stooge **s){
    ...
    for(int i = 0; i < 3; i++){
        ...
        if(s[i]->getMatch() == true && s[i]->getCardSuit(s[i]->getChoice()) > max){
            //set their choice to the max
            max = s[i]->getCardSuit(s[i]->getChoice());
            //set them as winner
            winner = i;
        }
        //now see if there was a heart played
        if(s[i]->getCardSuit(s[i]->getChoice()) > 38){
            //add 1 to the trick value
            tempscore += 1;
        }
        //last check if the Q Spades was played
        if(s[i]->getCardSuit(s[i]->getChoice()) == 36){
            tempscore += 13;
        }
    }

    //now check the player
   if(p.getMatch() == true && p.getCardSuit(p.getChoice()) > max){
    //set their choice to the max
    max = p.getCardSuit(p.getChoice());
    //set them as winner
    winner = 3;
    }
    //now see if there was a heart played
    if(p.getCardSuit(p.getChoice()) > 38){
        //add 1 to the trick value
        tempscore += 1;
    }
    //last check if the Q Spades was played
    if(p.getCardSuit(p.getChoice()) == 36){
        tempscore += 13;
    }
