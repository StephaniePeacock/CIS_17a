...
int main(int argc, char** argv) {
    ...
    p.setName(input);
    //let's get all the names for the STL stuff
    vector<string> names = {p.getName(),s[0]->getName(), s[1]->getName(), s[2]->getName()};
    vector<string>::iterator who;
    //finish intro
    cout << "Our players are ";
    for(who = names.begin(); who != names.end(); who++){
        cout << *who << "\t";
    }
    cout << endl << "Alright " << p.getName() << ", let's play Hearts! I'll deal." << endl << endl;
    /************************
     *                      *
     *    BEGIN GAME LOOP   *
     *                      *
